<template>
    <div>
        <header>
            <h2 class="text-gray-500 text-4xl font-semi-bold uppercase">Shop</h2>
        </header>
        <!-- items -->
        <div v-if="latestProducts.length">
            <h4 class="font-bold mt-12 pb-2 border-b border-gray-200">All Products</h4>
            ...
            <!-- cards -->
                <div class="card relative" v-for="product in latestProducts" :key="product.id">
                    ...
                    <div class="m-2 flex justify-evenly">
                        ...
                        <nuxt-link :to="{ name: 'product-category_slug-product_slug', params: {category_slug: `${ product.category_slug }`, product_slug: `${ product.slug }`} }" class="rounded-full py-1 px-2 text-xs font-bold cursor-pointer tracking-wider border-gray-500 md:border-2 text-gray hover:bg-gray-500 hover:text-white transition ease-out duration-500">
                        ...
                        <!-- <nuxt-link :to="{ name: 'product-category_slug-product_slug', params: {category_slug: 'someCategory', product_slug: 'someProduct'} }">Link to product</nuxt-link> -->
                    </div>
                </div>
            </div>
